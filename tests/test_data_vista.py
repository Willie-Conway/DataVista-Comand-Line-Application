...
class TestDataVista(unittest.TestCase):
    def setUp(self):
        self.app = DataVista()
        self.app.load_data('data/test_data_with_duplicates.csv')
    ...
    @patch('builtins.input', side_effect=[
        '2',    # Scale features? No (2)
        '2',    # Remove outliers? No (2)
        '3'     # Missing value handling skip
    ])
    ...
    @patch('builtins.input', side_effect=[
        '3'     # Missing value handling skip for cleaner
    ])
    def test_clean_data(self, mock_input):
        original_shape = self.app.data.shape
        self.app.clean_data()
        self.assertLess(self.app.data.shape[0], original_shape[0])
    ...
    @patch('builtins.input', side_effect=[
        'n',    # Name x and y axes? No
        'n'     # Add chart title? No
    ])
